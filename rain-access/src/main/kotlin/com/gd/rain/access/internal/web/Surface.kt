...
public class AccessEnforcementInterceptor(
    ...
    override fun preHandle(
        ...
        val lookup = declarations.of(handler.beanType, handler, handler.bean as? DeclaresItsOwnAccess, request.method, pattern)
        val declaration =
            when (lookup) {
                ...
                DeclarationLookup.Undeclared -> {
                    error(
                        "${request.method} $pattern declares no access; the start-up verification refuses that",
                    )
                }
            }
 ...
 * its access or is exempted by a [SurfaceExemption]; a mapping without a method stands for every method no other mapping
