...
/**
 * In the `prod` stage, while credentials travel in cookies, `rain.web.cors.allowed-origins` names no loopback host: an
 * allowed origin drives the credential surface with the caller's cookies, and a loopback origin is a page on whichever
 * machine the browser runs on.
 */
public class LoopbackOriginRule : CrossSectionRule {
    override val id: String = "rain.access.loopback-origins"
    override val reads: Set<String> = setOf(RainWebProperties.PREFIX, AccessProperties.PREFIX)

    override fun evaluate(
        sections: BoundSections,
        stage: DeploymentStage,
    ): RuleOutcome {
        if (stage != DeploymentStage.PROD) return RuleOutcome.Satisfied
        val access = sections.get(AccessProperties.PREFIX, AccessProperties::class)
        if (access.web.delivery == CredentialDelivery.BODY) return RuleOutcome.Satisfied
        val web = sections.get(RainWebProperties.PREFIX, RainWebProperties::class)
        val found =
            web.cors.allowedOrigins.mapIndexedNotNull { index, origin ->
                val path = "${RainWebProperties.CORS}.allowed-origins[$index]"
                if (origin == ANY_ORIGIN) return@mapIndexedNotNull null
                val host =
                    try {
                        URI(origin).host
                ...
                when {
                    host == null -> {
                        ConfigurationProblem(path, ProblemCode.NOT_EVALUATED, "is \"$origin\", which names no host this rule can read")
                    }
                    ...
                    Loopback.isLoopbackHost(host) -> {
                        ConfigurationProblem(
                            path,
                            ProblemCode.INVALID,
                            "is \"$origin\" in the ${stage.wire} stage while credentials travel in cookies; a loopback origin is a page " +
                                "on the browser's own machine, and it would drive this API with the caller's session",
                        )
                    }
        ...
        return if (found.isEmpty()) RuleOutcome.Satisfied else RuleOutcome.Violated(found)
    }
...
public class AccessConfigurationContributor : ConfigurationContributor {
    ...
    override val rules: List<CrossSectionRule> = listOf(LoopbackOriginRule())
}
