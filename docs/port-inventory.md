...
| resource/doc | `docs/backend/access/README.md` | docs/modules/access.md | ported | docs/modules/access.md: unknown_subject_type resolution, grants to active subjects, one password per subject by a partial unique index, usableHolderOf, revocation replay; Go layers, product role config and patch migration dropped |
| resource/doc | `docs/backend/access/accounts.md` | docs/modules/access.md | ported | docs/modules/access.md: directory active check per request and per rotation, identifier stored normalised, operator password set from the directory identifier; user CRUD DTOs, email parsing and product permissions dropped |
...
| resource/doc | `docs/backend/core/redis/README.md` | docs/modules/access.md (revocation server) | ported | docs/modules/access.md#revocation: eviction as a server property, noeviction check across cluster masters, attestation for managed Redis, dedicated rainRevocation/rainAttempts factories, readiness pings the store's own factory; cache tenants, shared-server waiver and fx hooks dropped |
