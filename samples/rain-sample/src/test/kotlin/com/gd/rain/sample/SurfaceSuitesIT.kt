...
class AuthorizationIT {
    ...
    fun `the system role cannot be renamed or removed, an ordinary role can, and no role made through the api is a system role`() {
        ...
        // A body cannot even ask for a system role: `system` is not a field the route reads.
        val asked = api.http.send("POST", "/v1/roles", """{"slug":"temporary","name":"Temporary","system":true}""", admin)
        assertThat(asked.statusCode() to asked.json()["errors"][0]["pointer"].asString()).isEqualTo(422 to "/system")
        val created = api.http.send("POST", "/v1/roles", """{"slug":"temporary","name":"Temporary"}""", admin)
