...
-- One index per declared query shape and scope — every ticket, and the tickets assigned to one agent (TicketPlanProofIT
-- proves each statement bounded by one). Under one scope, no two indexes serve one sort after nested equality columns, so
-- the planner never chooses between a bounded scan and a filtered one.
...
-- both scopes, filter[assignee][eq] and filter[status][eq], sort=-priority
...
-- assigned, filter[status][eq], sort=-createdAt
CREATE INDEX ix_tickets_assignee_status_created_at ON tickets (assignee, status, created_at, id);
