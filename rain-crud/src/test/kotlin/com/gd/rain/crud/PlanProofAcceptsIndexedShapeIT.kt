...
class PlanProofAcceptsIndexedShapeIT {
    ...
    private val lists = MountedResource("/books", setOf(CrudOperation.LIST, CrudOperation.COUNT), resource)
    private val scopes =
        listOf(
    ...
    fun `every statement of every declared shape is bounded on an empty table`() {
        val result = CrudPlanProof.verify(database.store, lists, scopes, database.dataSource)
        ...
        assertThat(result.statements.map { it.kind }.toSet()).containsExactlyInAnyOrder(*SHAPE_KINDS)
    ...
    fun `a table that holds rows or statistics is not evaluated, and every reason is named`() {
        ...
        assertThatThrownBy { CrudPlanProof.verify(database.store, lists, scopes, database.dataSource) }
