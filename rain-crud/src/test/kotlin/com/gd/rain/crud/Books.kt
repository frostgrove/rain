...
object Books {
    ...
    /** isbn is filterable but not selectable; every other field is both. Pass [includable] with the relations the resource declares. */
    fun rules(
        countCap: Long? = 50,
        ...
        QueryRules(
            filterable = FieldGrant.only("title", "shelf", "pages", "price", "publishedOn", "createdAt", "available", "isbn", "copies"),
            sortable = FieldGrant.only("title", "createdAt", "pages", "price"),
            selectable = FieldGrant.only("title", "shelf", "pages", "price", "publishedOn", "createdAt", "available", "copies"),
            includable = includable,
            searchFields = listOf("title"),
            pagination =
                Pagination(
                    defaultLimit = 10,
                    maxLimit = 50,
                    maxOffset = maxOffset,
                    cursorSorts = setOf(SortKey.NONE, SortKey.parse("-createdAt"), SortKey.parse("title")),
                    countCap = countCap,
                ),
            limits = limits,
        )
